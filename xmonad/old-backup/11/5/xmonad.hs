...
myKeys =
    [("M-" ++ myModifier ++ myKey, windows $ onCurrentScreen myFunction myWorkspace)
        | (myWorkspace, myKey) <- zip myWorkspaces (map show [1 :: Int ..])
     --   | (i, k) <- zip (myWorkspaces) (map show [1 .. 9 :: Int])  
        , let shiftAndView workspace = W.view workspace . W.shift workspace
        , let copyAndView workspace = W.view workspace . copy workspace
        , (myFunction, myModifier) <- [(W.view, ""), (W.shift, "S-"), (copy, "S-C-")]]    --use this when you dont want to switch automatically to the workspace you moved your window to
