...
myLayout = avoidStruts $ refocusLastLayoutHook $ trackFloating $ mouseResize $ windowArrange $ hiddenWindows $ T.toggleLayouts (floating) $ mkToggle (NBFULL ?? NOBORDERS ?? EOT) ( tiled ||| grid ||| magnify ||| tabs ||| noBorders Full ||| floating ||| twoPP ||| twoPPV)
  where
     -- tiled
