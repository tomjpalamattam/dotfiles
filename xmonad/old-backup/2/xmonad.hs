...
myStartupHook = do
    ...
    spawnOnce "dunst &"
    spawnOnce "sleep 4 && com.getmailspring.Mailspring -b &"
   -- spawnOnce "stalonetray"
   -- spawnOnce "stalonetray --icon-size=24 --kludges=force_icons_size &"
    spawnOnce "tint2 -c ~/.config/tint2/tint2rc-systray &"
    spawnOnce "picom --config ~/.config/picom/picom-xmonad.conf --backend glx --experimental-backends &"
    spawnOnce "nm-applet &"
...
myLayout = avoidStruts $ refocusLastLayoutHook $ trackFloating $ mouseResize $ windowArrange $ T.toggleLayouts (floating) $ mkToggle (NBFULL ?? NOBORDERS ?? EOT) ( tiled ||| grid ||| magnify ||| tabs ||| noBorders Full ||| floating ||| twoPP ||| twoPPV)
  where
     -- tiled
...
-- other possible keybindings: windows W.focusMaster, windows W.swapMaster, windows W.swapUp
     ...
     , ("C-b", spawn "brave --enable-features=VaapiVideoDecoder --use-gl=egl --disable-features=UseChromeOSDirectVideoDecoder") -- rofi
     , ("C-n", spawn "brave --incognito --enable-features=VaapiVideoDecoder --use-gl=egl --disable-features=UseChromeOSDirectVideoDecoder") -- rofi
--     , ("M-S-l", spawn "i3lock-fancy-rapid 8 3") -- lockscreen
 ...
 --  , ("M-C-r",  spawnDynamicSP "dyn2") -- bring dyn2 scratchpad window --deprecated
  ...
  --   , ("M-S-y", sequence_ [withAll' $ W.shiftWin " 2 ",  windows $ W.greedyView " 2 " ]) -- to switch a window or tabbed group of sublayout to 2nd workspace and also moves xmonad to workspace 2 --use with XMonad.Actions.WithAll --used with sublayout  
     , ("M-S-y", withFocused (sendMessage . mergeDir' W.focusDown'))
...
mergeDir' f = WithGroup g
  where
    g cs = do
        ...
        flip whenJust (sendMessage . XMonad.Layout.SubLayouts.Merge c . W.focus . f)
