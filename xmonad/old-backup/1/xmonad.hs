...
myLayout = avoidStruts $ refocusLastLayoutHook $ trackFloating $ mouseResize $ windowArrange $ T.toggleLayouts (floating) $ mkToggle (NBFULL ?? NOBORDERS ?? EOT) ( tiled ||| grid ||| magnify ||| tabs ||| noBorders Full ||| floating ||| twoP ||| twoPP ||| twoPPV ||| ifmax2 ||| tiled2)
  where
     -- tiled
